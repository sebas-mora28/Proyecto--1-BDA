...
app = Flask(__name__)
app.config['MONGO_URI'] = 'mongodb://localhost:{PORT_MONGO}/proyecto'.format(
    ...
    for club in clubs:
        ...
        if name == club['name'] and category == club['category']:
            ...
            for follower in followers:
                if id_user == follower["idU"]:
                    abort(404)
            db_clubs.update_one({'_id': ObjectId(club_id)}, {
                                '$push': {'followers': {'idU': id_user}}})
...
def get_club_count_by_category():
    ...
    categories = ["Deportes", "Artes", "Idiomas",
                  "Matematicas", "Ciencias", "Manualidades"]
    category_count = []
...
def top_3_subs():
    ...
    for user in users:
        user_clubs = db_clubs.find(
            {'followers': {'$elemMatch': {'idU': user['_id']}}})
        count_appeared = 0
...
def get_top_5_clubs():
    ...
    # Sort the count
    clubs_follower_count = sorted(
        clubs_follower_count, key=lambda x: x['followers'], reverse=True)
...
def get_bottom_3_clubs():
    ...
    # Sort the count
    clubs_follower_count = sorted(
        clubs_follower_count, key=lambda x: x['followers'])
